...
export const Field: React.FC = () => {
	...
	const handleBallCollision = (ballA: Ball, ballB: Ball) => {
		...
		if (distance < ballA.radius + ballB.radius) {
			...
			const v1x =
				ballA.vx * Math.cos(collisionAngle) +
				ballA.vy * Math.sin(collisionAngle)
			const v1y =
				ballA.vy * Math.cos(collisionAngle) -
				ballA.vx * Math.sin(collisionAngle)
			const v2x =
				ballB.vx * Math.cos(collisionAngle) +
				ballB.vy * Math.sin(collisionAngle)
			const v2y =
				ballB.vy * Math.cos(collisionAngle) -
				ballB.vx * Math.sin(collisionAngle)

			// Упругое столкновение с учетом массы
			const newV1x =
				((ballA.mass - ballB.mass) * v1x + 2 * ballB.mass * v2x) /
				(ballA.mass + ballB.mass)
			const newV2x =
				((ballB.mass - ballA.mass) * v2x + 2 * ballA.mass * v1x) /
				(ballA.mass + ballB.mass)

			// Преобразуем обратно в координаты X и Y
			const finalV1x =
				newV1x * Math.cos(collisionAngle) - v1y * Math.sin(collisionAngle)
			const finalV1y =
				newV1x * Math.sin(collisionAngle) + v1y * Math.cos(collisionAngle)
			const finalV2x =
				newV2x * Math.cos(collisionAngle) - v2y * Math.sin(collisionAngle)
			const finalV2y =
				newV2x * Math.sin(collisionAngle) + v2y * Math.cos(collisionAngle)
			...
			const overlapVector = {
				x: overlap * Math.cos(collisionAngle),
				y: overlap * Math.sin(collisionAngle),
			}
